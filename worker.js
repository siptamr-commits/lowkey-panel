...
const __WORKER_EXPORT__ = {
	async fetch(request, env, ctx) {
		...
		if (url.pathname === "/telegram-webhook" && request.method === "POST") {
			return await TelegramBot.handleWebhook(request, env, ctx);
		}
...
const TelegramBot = {
	...
	isAdmin(env, userId) {
		const ids = (env.TELEGRAM_ADMIN_IDS || "").split(",").map((s) => s.trim()).filter(Boolean);
		return ids.includes(String(userId));
	},
	async getSession(env, chatId) {
		try {
			const row = await env.DB.prepare("SELECT state, data FROM tg_sessions WHERE chat_id = ?").bind(String(chatId)).first();
	...
	async setSession(env, chatId, state, data = {}) {
		try {
			await env.DB.prepare(
				"INSERT INTO tg_sessions (chat_id, state, data, updated_at) VALUES (?, ?, ?, ?) ON CONFLICT(chat_id) DO UPDATE SET state = excluded.state, data = excluded.data, updated_at = excluded.updated_at"
			).bind(String(chatId), state, JSON.stringify(data || {}), Date.now()).run();
		} catch (e) {}
	},
	async clearSession(env, chatId) {
		try {
			await env.DB.prepare("DELETE FROM tg_sessions WHERE chat_id = ?").bind(String(chatId)).run();
		} catch (e) {}
	},
	async isChannelMember(env, userId) {
		const channel = env.TELEGRAM_CHANNEL;
		if (!channel) return true;
		const res = await this.call(env, "getChatMember", { chat_id: channel, user_id: userId });
		if (!res || !res.ok || !res.result) return false;
		const status = res.result.status;
		return status === "member" || status === "administrator" || status === "creator";
	},
	joinGateKeyboard(env) {
		const channel = (env.TELEGRAM_CHANNEL || "").replace(/^@/, "");
		return {
			inline_keyboard: [
	...
	mainMenuKeyboard(env, userId) {
		...
		if (this.isAdmin(env, userId)) {
			rows.push([{ text: "\ud83d\udee0 \u067e\u0646\u0644 \u0645\u062f\u06cc\u0631\u06cc\u062a", callback_data: "admin_menu" }]);
		}
	...
	async sendMainMenu(env, chatId, userId, extraText = null) {
		await this.call(env, "sendMessage", {
			chat_id: chatId,
			text: extraText || this.welcomeText(),
			parse_mode: "Markdown",
			reply_markup: this.mainMenuKeyboard(env, userId),
		});
	},
	genUsername(userId) {
	...
	async createBotUser(env, request, chatId, userId, opts = {}) {
		...
		let defaultPort = "443";
		try {
			const portSetting = await env.DB.prepare("SELECT value FROM settings WHERE key = 'ports'").first();
			if (portSetting && portSetting.value) {
				const parsed = JSON.parse(portSetting.value);
				if (Array.isArray(parsed) && parsed.length) defaultPort = String(parsed[0]);
			}
		} catch (e) {}
	...
	async myConfigsText(env, chatId, request) {
		const url = new URL(request.url);
		const host = url.hostname;
		const { results } = await env.DB.prepare("SELECT username, limit_gb, used_gb, expiry_days, created_at, is_active FROM users WHERE tg_chat_id = ? ORDER BY created_at DESC").bind(String(chatId)).all();
		if (!results || results.length === 0) {
			return "\u0647\u0646\u0648\u0632 \u0647\u06cc\u0686 \u06a9\u0627\u0646\u0641\u06cc\u06af\u06cc \u0646\u0633\u0627\u062e\u062a\u06cc. \u0627\u0632 \u0645\u0646\u0648 \u00ab\u2795 \u0633\u0627\u062e\u062a \u06a9\u0627\u0646\u0641\u06cc\u06af \u062c\u062f\u06cc\u062f\u00bb \u0631\u0648 \u0628\u0632\u0646.";
		}
		let text = "\ud83d\udcc4 *\u06a9\u0627\u0646\u0641\u06cc\u06af\u200c\u0647\u0627\u06cc \u062a\u0648:*\n\n";
		for (const u of results) {
			const daysLeft = u.expiry_days ? Math.max(0, u.expiry_days - Math.floor((Date.now() - u.created_at) / 86400000)) : "\u221e";
			const status = u.is_active ? "\ud83d\udfe2 \u0641\u0639\u0627\u0644" : "\ud83d\udd34 \u063a\u06cc\u0631\u0641\u0639\u0627\u0644";
			const usedDisplay = Number(u.used_gb || 0).toFixed(2);
			text += "\u2022 `" + u.username + "` \u2014 " + usedDisplay + " / " + u.limit_gb + " GB \u2014 " + daysLeft + " \u0631\u0648\u0632 \u0645\u0648\u0646\u062f\u0647 \u2014 " + status + "\n";
			text += "  \u0644\u06cc\u0646\u06a9: `https://" + host + "/sub/" + u.username + "`\n\n";
		}
		return text;
	},

	// ===== Admin: list/detail/edit/delete =====
	adminMenuKeyboard() {
		return {
			inline_keyboard: [
	...
	async sendUserList(env, chatId, page) {
		...
		const rows = results.map((u) => {
			const dot = u.is_active ? "\ud83d\udfe2" : "\ud83d\udd34";
			return [{ text: dot + " " + u.username + " (" + Number(u.used_gb || 0).toFixed(1) + "/" + u.limit_gb + "GB)", callback_data: "admin_user:" + u.username }];
		});
		const nav = [];
		if (page > 0) nav.push({ text: "\u25c0\ufe0f \u0642\u0628\u0644\u06cc", callback_data: "admin_list:" + (page - 1) });
		if (offset + results.length < total) nav.push({ text: "\u0628\u0639\u062f\u06cc \u25b6\ufe0f", callback_data: "admin_list:" + (page + 1) });
		if (nav.length) rows.push(nav);
		rows.push([{ text: "\u2b05\ufe0f \u0628\u0627\u0632\u06af\u0634\u062a", callback_data: "admin_menu" }]);
		await this.call(env, "sendMessage", {
			chat_id: chatId,
			text: "\ud83d\udccb *\u0644\u06cc\u0633\u062a \u06a9\u0627\u0631\u0628\u0631\u0627\u0646* (" + total + " \u0646\u0641\u0631) \u2014 \u0635\u0641\u062d\u0647\u0654 " + (page + 1),
			parse_mode: "Markdown",
			reply_markup: { inline_keyboard: rows },
		});
	},
	async userDetailKeyboard(username) {
		return {
			inline_keyboard: [
				[
					{ text: "\ud83d\udcc8 \u062a\u063a\u06cc\u06cc\u0631 \u062d\u062c\u0645", callback_data: "admin_edit_limit:" + username },
				...
				[
					...
					{ text: "\ud83d\uddd1 \u062d\u0630\u0641", callback_data: "admin_delete_confirm:" + username },
				],
	...
	async sendUserDetail(env, chatId, username) {
		...
		if (!u) {
			await this.call(env, "sendMessage", { chat_id: chatId, text: "\u06a9\u0627\u0631\u0628\u0631 \u067e\u06cc\u062f\u0627 \u0646\u0634\u062f." });
			return;
		}
		const daysLeft = u.expiry_days ? Math.max(0, u.expiry_days - Math.floor((Date.now() - u.created_at) / 86400000)) : "\u221e";
		const status = u.is_active ? "\ud83d\udfe2 \u0641\u0639\u0627\u0644" : "\ud83d\udd34 \u063a\u06cc\u0631\u0641\u0639\u0627\u0644";
		const source = u.tg_user_id ? "\ud83e\udd16 \u0631\u0628\u0627\u062a" : "\ud83c\udf10 \u067e\u0646\u0644 \u0648\u0628";
		const text =
			"\ud83d\udc64 *" + u.username + "*\n" +
			"\u0648\u0636\u0639\u06cc\u062a: " + status + "\n" +
			"\u0645\u0646\u0628\u0639: " + source + "\n" +
	...
	async handleWebhook(request, env, ctx) {
		...
		try {
			if (update.callback_query) {
				...
				const data = cq.data;
				await this.call(env, "answerCallbackQuery", { callback_query_id: cq.id });

				if (data === "check_join") {
					const joined = await this.isChannelMember(env, userId);
					if (joined) {
						await this.sendMainMenu(env, chatId, userId, "\u2705 \u0639\u0636\u0648\u06cc\u062a\u062a \u062a\u0623\u06cc\u06cc\u062f \u0634\u062f!\n\n" + this.welcomeText());
					} else {
						await this.call(env, "sendMessage", { chat_id: chatId, text: "\u0647\u0646\u0648\u0632 \u0639\u0636\u0648 \u06a9\u0627\u0646\u0627\u0644 \u0646\u0634\u062f\u06cc.", reply_markup: this.joinGateKeyboard(env) });
					}
					return new Response("ok");
				}
				...
				const joined = await this.isChannelMember(env, userId);
				if (!joined) {
					await this.call(env, "sendMessage", { chat_id: chatId, text: "\u0627\u0648\u0644 \u0639\u0636\u0648 \u06a9\u0627\u0646\u0627\u0644 \u0634\u0648:", reply_markup: this.joinGateKeyboard(env) });
					return new Response("ok");
				}
				...
				if (data === "new_config") {
					...
					await this.call(env, "sendMessage", { chat_id: chatId, text: "\ud83d\udd12 \u0644\u0648\u06a9\u06cc \u2014 \u067e\u0631\u0648\u06a9\u0633\u06cc \u0627\u0645\u0646 \u0648 \u0634\u062e\u0635\u06cc.\n\n\u06a9\u0627\u0646\u0627\u0644: @" + (env.TELEGRAM_CHANNEL || "lowkey878").replace(/^@/, "") });
					await this.sendMainMenu(env, chatId, userId);
				} else if (data === "admin_menu") {
					if (!this.isAdmin(env, userId)) {
						await this.call(env, "sendMessage", { chat_id: chatId, text: "\u26d4\ufe0f \u0627\u06cc\u0646 \u0628\u062e\u0634 \u0641\u0642\u0637 \u0628\u0631\u0627\u06cc \u0645\u062f\u06cc\u0631\u0647." });
						await this.sendMainMenu(env, chatId, userId);
					} else {
						await this.sendAdminMenu(env, chatId);
					}
				} else if (data.startsWith("admin_list:")) {
					if (!this.isAdmin(env, userId)) return new Response("ok");
					const page = parseInt(data.split(":")[1], 10) || 0;
					await this.sendUserList(env, chatId, page);
				} else if (data.startsWith("admin_user:")) {
					if (!this.isAdmin(env, userId)) return new Response("ok");
					const username = data.slice("admin_user:".length);
					await this.sendUserDetail(env, chatId, username);
				} else if (data.startsWith("admin_edit_limit:")) {
					if (!this.isAdmin(env, userId)) return new Response("ok");
					const username = data.slice("admin_edit_limit:".length);
					await this.setSession(env, chatId, "await_edit_limit", { username });
					await this.call(env, "sendMessage", { chat_id: chatId, text: "\u062d\u062c\u0645 \u062c\u062f\u06cc\u062f \u0631\u0648 \u0628\u0647 \u06af\u06cc\u06af\u0627\u0628\u0627\u06cc\u062a \u0628\u0641\u0631\u0633\u062a (\u0645\u062b\u0644\u0627 20):" });
				} else if (data.startsWith("admin_edit_days:")) {
					if (!this.isAdmin(env, userId)) return new Response("ok");
					...
					await env.DB.prepare("UPDATE users SET used_gb = 0, used_req = 0, is_active = 1 WHERE username = ?").bind(username).run();
					await this.call(env, "sendMessage", { chat_id: chatId, text: "\u2705 \u0645\u0635\u0631\u0641 \u0631\u06cc\u0633\u062a \u0634\u062f." });
					await this.sendUserDetail(env, chatId, username);
				} else if (data.startsWith("admin_toggle:")) {
					if (!this.isAdmin(env, userId)) return new Response("ok");
					const username = data.slice("admin_toggle:".length);
					await env.DB.prepare("UPDATE users SET is_active = CASE WHEN is_active = 1 THEN 0 ELSE 1 END WHERE username = ?").bind(username).run();
					await this.sendUserDetail(env, chatId, username);
				} else if (data.startsWith("admin_getlink:")) {
					if (!this.isAdmin(env, userId)) return new Response("ok");
					const username = data.slice("admin_getlink:".length);
					const url = new URL(request.url);
					await this.call(env, "sendMessage", { chat_id: chatId, text: "\ud83d\udd17 `https://" + url.hostname + "/sub/" + username + "`", parse_mode: "Markdown" });
				} else if (data.startsWith("admin_delete_confirm:")) {
					if (!this.isAdmin(env, userId)) return new Response("ok");
					const username = data.slice("admin_delete_confirm:".length);
					await this.call(env, "sendMessage", {
						chat_id: chatId,
						text: "\u26a0\ufe0f \u0627\u0632 \u062d\u0630\u0641 `" + username + "` \u0645\u0637\u0645\u0626\u0646\u06cc\u061f \u0627\u06cc\u0646 \u063a\u06cc\u0631\u0642\u0627\u0628\u0644 \u0628\u0631\u06af\u0634\u062a\u0647.",
						parse_mode: "Markdown",
						reply_markup: { inline_keyboard: [[{ text: "\u2705 \u0628\u0644\u0647\u060c \u062d\u0630\u0641 \u06a9\u0646", callback_data: "admin_delete:" + username }, { text: "\u274c \u0627\u0646\u0635\u0631\u0627\u0641", callback_data: "admin_user:" + username }]] },
					});
				} else if (data.startsWith("admin_delete:")) {
					if (!this.isAdmin(env, userId)) return new Response("ok");
					const username = data.slice("admin_delete:".length);
					await env.DB.prepare("DELETE FROM users WHERE username = ?").bind(username).run();
					await this.call(env, "sendMessage", { chat_id: chatId, text: "\ud83d\uddd1 \u06a9\u0627\u0631\u0628\u0631 \u062d\u0630\u0641 \u0634\u062f." });
					await this.sendAdminMenu(env, chatId);
				} else if (data === "admin_add") {
					if (!this.isAdmin(env, userId)) return new Response("ok");
					await this.setSession(env, chatId, "await_add_username", {});
					await this.call(env, "sendMessage", { chat_id: chatId, text: "\u06cc\u0648\u0632\u0631\u0646\u06cc\u0645 \u06a9\u0627\u0631\u0628\u0631 \u062c\u062f\u06cc\u062f \u0631\u0648 \u0628\u0641\u0631\u0633\u062a (\u0641\u0642\u0637 \u062d\u0631\u0648\u0641 \u0648 \u0639\u062f\u062f):" });
			...
			if (update.message) {
				...
				if (text === "/start") {
					...
					if (!joined) {
						await this.call(env, "sendMessage", {
							chat_id: chatId,
							text: "\ud83d\udc4b \u062e\u0648\u0634 \u0627\u0648\u0645\u062f\u06cc! \u0627\u0648\u0644 \u0639\u0636\u0648 \u06a9\u0627\u0646\u0627\u0644 \u0634\u0648\u060c \u0628\u0639\u062f \u0631\u0648\u06cc \u00ab\u2705 \u0639\u0636\u0648 \u0634\u062f\u0645\u00bb \u0628\u0632\u0646:",
							reply_markup: this.joinGateKeyboard(env),
						});
					} else {
				...
				if (session.state && this.isAdmin(env, userId)) {
					...
					if (session.state === "await_edit_days") {
						...
						const nowTime = Date.now();
						await env.DB.prepare("UPDATE users SET expiry_days = ?, created_at = ? WHERE username = ?").bind(val, nowTime, session.data.username).run();
						await this.clearSession(env, chatId);
						await this.call(env, "sendMessage", { chat_id: chatId, text: "\u2705 \u0627\u0639\u062a\u0628\u0627\u0631 \u0628\u0647 \u0631\u0648\u0632 \u0634\u062f (\u0627\u0632 \u0627\u0645\u0631\u0648\u0632 \u0634\u0645\u0627\u0631\u0634 \u0645\u06cc\u0634\u0647)." });
						await this.sendUserDetail(env, chatId, session.data.username);
						return new Response("ok");
					}
					if (session.state === "await_add_username") {
						...
						if (!uname) {
							await this.call(env, "sendMessage", { chat_id: chatId, text: "\u06cc\u0648\u0632\u0631\u0646\u06cc\u0645 \u0646\u0627\u0645\u0639\u062a\u0628\u0631. \u062f\u0648\u0628\u0627\u0631\u0647 \u0628\u0641\u0631\u0633\u062a." });
							return new Response("ok");
						}
						const existing = await env.DB.prepare("SELECT username FROM users WHERE username = ?").bind(uname).first();
						if (existing) {
					...
					if (session.state === "await_search") {
						const q = text.replace(/[^a-zA-Z0-9_-]/g, "");
						await this.clearSession(env, chatId);
						const { results } = await env.DB.prepare("SELECT username FROM users WHERE username LIKE ? LIMIT 10").bind("%" + q + "%").all();
						if (!results || results.length === 0) {
							await this.call(env, "sendMessage", { chat_id: chatId, text: "\u0686\u06cc\u0632\u06cc \u067e\u06cc\u062f\u0627 \u0646\u0634\u062f." });
							await this.sendAdminMenu(env, chatId);
							return new Response("ok");
						}
						const rows = results.map((u) => [{ text: u.username, callback_data: "admin_user:" + u.username }]);
						rows.push([{ text: "\u2b05\ufe0f \u0628\u0627\u0632\u06af\u0634\u062a", callback_data: "admin_menu" }]);
						await this.call(env, "sendMessage", { chat_id: chatId, text: "\ud83d\udd0d \u0646\u062a\u0627\u06cc\u062c \u062c\u0633\u062a\u062c\u0648:", reply_markup: { inline_keyboard: rows } });
						return new Response("ok");
					}
				}
...
const Router = {
	...
	async handleApi(request, url, env, ctx) {
		...
		if (url.pathname === "/api/update-panel" && request.method === "POST") {
			return new Response(JSON.stringify({ error: "این پنل دیگر از گیت‌هاب سازنده‌ی اصلی به‌روزرسانی نمی‌شود (برای حفظ سفارشی‌سازی‌های شما)." }), { status: 400, headers: { "Content-Type": "application/json" } });
		}
		if (false && url.pathname === "/api/update-panel" && request.method === "POST") {
			...
			try {
				const cfHeaders = {
					...
					"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) LowkeyPanel/1.0"
				};
				if (!currentAccountId) {
		...
		schemaPromise = (async () => {
			...
			try {
				...
				const colsToAdd = [
					...
					{ name: "tg_chat_id", def: "TEXT DEFAULT NULL" },
					{ name: "tg_user_id", def: "TEXT DEFAULT NULL" },
...
const SubscriptionService = {
	async generateText(user, host) {
		...
		const links = [];
		const m2 = decodeURIComponent("%F0%9F%9A%80%40lowkey878%F0%9F%9A%80");
				...
				if (nowTime - lastCheck >= 20000) {
					...
					if (!user || user.is_active === 0) {
						...
						if (!isExpired && clientIP && clientIP !== "unknown") {
							...
							const wasPresent = !!activeIps[clientIP];
							if (!wasPresent) {
								...
								hasChanges = true;
								if (user.ip_limit && user.ip_limit > 0) {
									...
									if (sortedIps.indexOf(clientIP) >= user.ip_limit) isIpLimitExpired = true;
								}
							}
...
<style>
    /* ===== lowkey identity ===== */
    body { background: #f6f7f8; background-image: radial-gradient(circle at 15% 0%, rgba(245,194,66,0.05), transparent 40%); }
    .dark body { background: #050505; background-image: radial-gradient(circle at 15% 0%, rgba(255,216,111,0.10), transparent 45%); }
    ...
    ::selection { background: rgba(245,194,66,0.25); }
	...
	tailwind.config = {
		...
		theme: {
			extend: {
				...
				colors: { amoled: { bg: '#050505', card: '#0b0b0f', input: '#111114', border: '#3a3320' } }
			}
		}
	}
</script>`;
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border border-t-2 border-t-yellow-500 rounded-sm shadow-sm p-8 text-center flex flex-col items-center gap-4">
        <div class="p-4 bg-yellow-50 dark:bg-yellow-900/20 text-yellow-500 rounded-full mb-2">
        ...
        <p class="text-sm text-gray-600 dark:text-gray-400 leading-relaxed mt-2">
            ...
            <span class="inline-block px-2 py-1 bg-gray-100 dark:bg-amoled-input border border-gray-200 dark:border-zinc-800 rounded-sm font-mono text-yellow-500 font-bold mx-1 shadow-sm" dir="ltr">/panel</span> 
            را به انتهای آدرس مرورگر خود اضافه کنید.
        </p>
        <button onclick="window.location.href='/panel'" class="mt-4 w-full py-2.5 bg-yellow-600 hover:bg-yellow-700 text-white dark:bg-yellow-600 dark:hover:bg-yellow-700 dark:text-white font-medium rounded-sm text-sm transition-colors duration-200 shadow-sm font-bold">
            ورود به پـنـل
        </button>
    </div>
</body>
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border border-t-2 border-t-yellow-500 rounded-sm shadow-sm p-6">
        <h2 class="text-xl font-bold mb-2 text-center text-yellow-600 dark:text-yellow-400">تنظیم رمز عبور جدید</h2>
        ...
        <form onsubmit="handleSetup(event)" class="space-y-4">
            <div>
                ...
                <input type="password" id="password" class="w-full px-3 py-2 bg-gray-50 dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-sm focus:outline-none focus:ring-2 focus:ring-yellow-500 text-sm text-center font-mono" required minlength="4">
            </div>
            <div>
                <label class="block text-sm font-medium mb-1.5">تکرار رمز عبور</label>
                <input type="password" id="confirm-password" class="w-full px-3 py-2 bg-gray-50 dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-sm focus:outline-none focus:ring-2 focus:ring-yellow-500 text-sm text-center font-mono" required minlength="4">
            </div>
            <button type="submit" id="submit-btn" class="w-full py-2.5 bg-yellow-600 hover:bg-yellow-700 text-white dark:bg-yellow-600 dark:hover:bg-yellow-700 dark:text-white font-medium rounded-sm text-sm transition font-bold">ثبت و ورود</button>
        </form>
    </div>
    ${COMMON_TOAST_HTML}
    <script>
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border border-t-2 border-t-yellow-500 rounded-sm shadow-sm p-6">
        <div id="login-section">
            <h2 class="text-xl font-bold mb-6 text-center text-yellow-600 dark:text-yellow-400">ورود به پـنـل مدیریت</h2>
            <form onsubmit="handleLogin(event)" class="space-y-4">
                <div>
                    <label class="block text-sm font-medium mb-1.5">رمز عبور</label>
                    <input type="password" id="password" class="w-full px-3 py-2 bg-gray-50 dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-sm focus:outline-none focus:ring-2 focus:ring-yellow-500 text-sm text-center font-mono" required>
                </div>
                <button type="submit" id="submit-btn" class="w-full py-2.5 bg-yellow-600 hover:bg-yellow-700 text-white dark:bg-yellow-600 dark:hover:bg-yellow-700 dark:text-white font-medium rounded-sm text-sm transition font-bold">ورود</button>
            </form>
            <div class="mt-4 text-center">
                <button onclick="toggleRecovery(true)" class="text-xs text-yellow-500 hover:text-yellow-600 transition font-medium">بازیابی رمز پـنـل</button>
            </div>
        </div>
        <div id="recovery-section" class="hidden">
            ...
            <form onsubmit="handleRecovery(event)" class="space-y-4">
                ...
                <div class="flex gap-2 pt-2">
                    ...
                    <button type="submit" id="recover-btn" class="w-2/3 py-2.5 bg-yellow-600 hover:bg-yellow-700 text-white dark:bg-yellow-600 dark:hover:bg-yellow-700 dark:text-white font-medium rounded-sm text-sm transition font-bold">بازیابی رمز پـنـل</button>
                </div>
            </form>
        </div>
    </div>
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen transition-colors duration-200">
    <header class="border-b border-gray-200 dark:border-amoled-border border-t-2 border-t-yellow-500 bg-white dark:bg-amoled-card px-4 py-4">
        <div class="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
            <div class="flex flex-row flex-wrap justify-center items-center gap-3 w-full md:w-auto">
                <h1 class="text-lg font-bold flex items-center gap-2" dir="ltr">
                    ...
                    L O W K E Y
                    <span id="panel-version" class="text-xs px-2 py-0.5 font-semibold bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400 rounded-full">v1.5.10</span>
                </h1>
            </div>
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
    <div class="bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border border-l-[3px] border-l-yellow-500 rounded-sm p-2.5 shadow-sm flex flex-col justify-center gap-1 hover:shadow-md transition duration-300 relative overflow-hidden group min-h-[64px]">
        <div class="absolute -right-4 -bottom-4 w-16 h-16 bg-yellow-500/10 rounded-full blur-xl group-hover:scale-150 transition duration-500"></div>
        <div class="flex items-center justify-between relative z-10">
            ...
            <div class="p-1 bg-yellow-50 dark:bg-yellow-950/30 text-yellow-600 dark:text-yellow-400 rounded-sm flex-shrink-0">
        ...
        <div class="flex items-end justify-between relative z-10 w-full mt-0.5">
            ...
            <span class="text-[9px] text-yellow-500 dark:text-yellow-400 flex items-center gap-1 font-medium whitespace-nowrap leading-none mb-0.5">
                <span class="w-1 h-1 bg-yellow-500 rounded-full animate-ping"></span>
    ...
    <div class="bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border border-l-[3px] border-l-yellow-500 rounded-sm p-2.5 shadow-sm flex flex-col justify-center gap-1 hover:shadow-md transition duration-300 relative overflow-hidden group min-h-[64px]">
        ...
        <div class="flex items-end justify-between relative z-10 w-full mt-0.5">
            <div class="text-lg font-black text-yellow-600 dark:text-yellow-400 transition-all whitespace-nowrap leading-none" id="stat-total-usage">0 GB</div>
            <span class="text-[9px] text-yellow-500 dark:text-yellow-400 flex items-center gap-0.5 font-medium whitespace-nowrap leading-none mb-0.5">
        ...
        <div class="mb-5 flex flex-col md:flex-row gap-2 justify-between items-center bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-sm p-2 shadow-sm">
            <div class="relative w-full md:w-80">
                <input type="text" id="search-input" oninput="filterAndRenderUsers()" placeholder="جستجوی نام کاربری یا UUID..." class="w-full pl-3 pr-8 py-1.5 bg-gray-50 dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-sm focus:outline-none focus:ring-2 focus:ring-yellow-500 text-xs">
            ...
            <div class="flex items-center gap-2 w-full md:w-auto">
                <select id="filter-status" onchange="filterAndRenderUsers()" class="flex-1 min-w-0 px-2 py-1.5 bg-gray-50 dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-sm text-xs focus:outline-none focus:ring-2 focus:ring-yellow-500 text-gray-700 dark:text-zinc-300 cursor-pointer truncate">
                ...
                <select id="sort-users" onchange="filterAndRenderUsers()" class="flex-1 min-w-0 px-2 py-1.5 bg-gray-50 dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-sm text-xs focus:outline-none focus:ring-2 focus:ring-yellow-500 text-gray-700 dark:text-zinc-300 cursor-pointer truncate">
		...
		<div class="flex items-center justify-between mb-4">
			<h2 class="text-lg font-bold text-gray-800 dark:text-zinc-200 flex items-center gap-2"><span class="w-1.5 h-4 bg-yellow-500 rounded-sm inline-block"></span>لیست کاربران</h2>
			<button onclick="openCreateModal()" class="p-2 rounded-sm bg-yellow-600 hover:bg-yellow-700 transition-all duration-200 text-white shadow-sm hover:shadow">
            ...
            <table class="w-full text-right border-collapse">
                <thead>
                    <tr class="bg-gray-100 dark:bg-zinc-900/50 border-b border-gray-200 dark:border-amoled-border text-[10px] font-mono uppercase tracking-wide text-gray-500 dark:text-gray-400 text-center">
                        <th class="p-2 w-10 text-center"><input type="checkbox" id="select-all-users" onchange="toggleSelectAllUsers(this)" class="w-5 h-5 rounded-sm border-2 border-gray-300 dark:border-zinc-700 text-yellow-600 bg-white dark:bg-zinc-800 checked:bg-yellow-600 checked:border-yellow-600 focus:ring-yellow-500/50 focus:ring-offset-0 transition-all duration-200 cursor-pointer hover:scale-105 active:scale-95"></th>
...
<div id="usage-warning-modal" class="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-orange-500/50 rounded-sm shadow-md overflow-hidden p-6 text-center transition-all transform duration-300 opacity-0 scale-95 ease-out">
        ...
        <button onclick="closeUsageWarning()" class="w-full py-3.5 bg-yellow-600 hover:bg-yellow-700 text-white dark:bg-yellow-600 dark:hover:bg-yellow-700 dark:text-white font-black rounded-sm text-sm transition duration-300 shadow-sm">
            متوجه شدم
        </button>
    </div>
</div>
    <div id="user-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 opacity-0 pointer-events-none transition-opacity duration-200 ease-out">
        <div id="user-modal-card" class="w-full max-w-xl lg:max-w-[1200px] bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border border-t-2 border-t-yellow-500 rounded-sm shadow-sm overflow-hidden transition-[opacity,transform] duration-200 opacity-0 scale-95 ease-out flex flex-col max-h-[90vh] transform-gpu" style="will-change: transform, opacity;">
            ...
            <form id="create-user-form" class="p-4 flex flex-col overflow-y-auto flex-1 overscroll-contain" style="-webkit-overflow-scrolling: touch; transform: translate3d(0,0,0); will-change: scroll-position, transform;" onsubmit="handleFormSubmit(event)">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-6 flex-1">
                        ...
                        <div class="space-y-2.5">
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" id="input-name" oninput="this.value = this.value.replace(/[^a-zA-Z0-9_-]/g, '')" placeholder="LOW_KEY" maxlength="32" class="w-full pl-3 pr-9 py-1.5 bg-gray-50 dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-sm focus:outline-none focus:ring-2 focus:ring-yellow-500/50 text-xs font-semibold text-gray-800 dark:text-zinc-100 placeholder-gray-400/80 transition" required>
                                </div>
                            </div>
                            <div class="grid grid-cols-2 gap-2.5">
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="number" id="input-limit" min="0" step="any" placeholder="نامحدود" class="w-full pl-3 pr-9 py-1.5 bg-gray-50 dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-sm focus:outline-none focus:ring-2 focus:ring-yellow-500/50 text-xs font-semibold text-gray-800 dark:text-zinc-100 placeholder-gray-400/80 transition">
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="number" id="input-expiry" min="0" placeholder="نامحدود" class="w-full pl-3 pr-9 py-1.5 bg-gray-50 dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-sm focus:outline-none focus:ring-2 focus:ring-yellow-500/50 text-xs font-semibold text-gray-800 dark:text-zinc-100 placeholder-gray-400/80 transition">
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="number" id="input-req-limit" min="0" placeholder="نامحدود" class="w-full pl-3 pr-9 py-1.5 bg-gray-50 dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-sm focus:outline-none focus:ring-2 focus:ring-yellow-500/50 text-xs font-semibold text-gray-800 dark:text-zinc-100 placeholder-gray-400/80 transition">
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="number" id="input-ip-limit" min="0" placeholder="نامحدود" class="w-full pl-3 pr-9 py-1.5 bg-gray-50 dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-sm focus:outline-none focus:ring-2 focus:ring-yellow-500/50 text-xs font-semibold text-gray-800 dark:text-zinc-100 placeholder-gray-400/80 transition">
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="flex flex-col gap-3 border border-gray-100 dark:border-amoled-border p-3 rounded-sm bg-gray-50 dark:bg-amoled-input">
                            ...
                            <div id="auto-reset-inputs-container" class="grid grid-cols-2 gap-2 transition-all duration-300 pt-2 border-t border-gray-100 dark:border-amoled-border opacity-50 pointer-events-none">
                                <div>
                                    ...
                                    <input type="number" id="input-auto-reset-vol" min="1" placeholder="خالی = بدون تمدید" class="w-full px-2 py-1.5 bg-gray-50 dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-sm focus:outline-none focus:ring-2 focus:ring-yellow-500/50 text-xs font-mono text-center text-gray-800 dark:text-zinc-100 transition" dir="ltr" disabled>
                                </div>
                                <div>
                                    ...
                                    <input type="number" id="input-auto-reset-req" min="1" placeholder="خالی = بدون تمدید" class="w-full px-2 py-1.5 bg-gray-50 dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-sm focus:outline-none focus:ring-2 focus:ring-yellow-500/50 text-xs font-mono text-center text-gray-800 dark:text-zinc-100 transition" dir="ltr" disabled>
                                </div>
                            </div>
                        </div>
                        <div class="grid grid-cols-2 gap-3">
                            <div class="p-3 bg-gray-50 dark:bg-amoled-input border border-gray-200/60 dark:border-amoled-border rounded-sm shadow-sm">
                                ...
                                <div id="frag-inputs-container" class="grid grid-cols-2 gap-1.5 transition-all duration-300">
                                    <input type="text" id="input-frag-len" placeholder="Len" value="200-3000" dir="ltr" class="w-full px-1.5 py-1 bg-white dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-sm focus:outline-none focus:ring-1 focus:ring-yellow-500 text-[10px] font-mono text-center text-gray-800 dark:text-zinc-100">
                                    <input type="text" id="input-frag-int" placeholder="Int" value="1-2" dir="ltr" class="w-full px-1.5 py-1 bg-white dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-sm focus:outline-none focus:ring-1 focus:ring-yellow-500 text-[10px] font-mono text-center text-gray-800 dark:text-zinc-100">
                                </div>
                            </div>
                            <div class="p-3 bg-gray-50 dark:bg-amoled-input border border-gray-200/60 dark:border-amoled-border rounded-sm shadow-sm">
                                <label class="block text-[10px] font-bold text-gray-500 dark:text-zinc-400 mb-1.5 uppercase tracking-wider">Fingerprint</label>
                                <div class="relative">
                                    <select id="fingerprint-select" class="w-full px-2 py-1.5 bg-white dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-sm focus:outline-none focus:ring-1 focus:ring-yellow-500 text-[10px] font-semibold text-gray-700 dark:text-zinc-300 cursor-pointer appearance-none">
                    ...
                    <div class="flex flex-col pt-4 lg:pt-0 border-t-2 lg:border-t-0 lg:border-x-2 border-gray-300 dark:border-amoled-border lg:px-4 h-full">
                        ...
                        <div class="grid grid-cols-2 gap-2 md:gap-4">
                            <div class="p-3 bg-gray-50 dark:bg-amoled-input border border-gray-200/60 dark:border-amoled-border rounded-sm shadow-sm flex flex-col">
                                <div class="flex items-center gap-1.5 mb-2">
                                    <span class="flex h-2 w-2 rounded-full bg-yellow-500 shadow-sm"></span>
                                    <span class="text-[11px] font-bold text-yellow-600 dark:text-yellow-400">🔒TLS PORT</span>
                                </div>
                        ...
                        <div class="mt-4 p-3 bg-gray-50 dark:bg-amoled-input border border-gray-200/60 dark:border-amoled-border rounded-sm shadow-sm">
                            ...
                            <input type="text" id="input-custom-ports" placeholder="8080 2096 5000" dir="ltr" class="w-full px-2 py-2 bg-white dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-sm focus:outline-none focus:ring-2 focus:ring-yellow-500/50 text-xs font-mono text-left text-gray-800 dark:text-zinc-100 transition">
                        </div>
                        <div class="flex flex-col flex-1 mt-4 pt-4 border-t-2 border-gray-300 dark:border-amoled-border">
                            ...
                            <textarea id="input-ips" placeholder="104.16.0.1" class="w-full h-full min-h-[80px] flex-1 px-3 py-2.5 bg-gray-50 dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-sm focus:outline-none focus:ring-2 focus:ring-yellow-500/50 text-xs font-mono text-gray-800 dark:text-zinc-100 placeholder-gray-400/80 transition resize-none"></textarea>
                        </div>
                    </div>
                    <div class="flex flex-col gap-2 pt-4 lg:pt-0 border-t-2 lg:border-t-0 border-gray-300 dark:border-amoled-border justify-between">
                        <div class="flex flex-col flex-1">
                            ...
                            <div class="grid grid-cols-2 gap-2 mb-2 w-full">
                                ...
                                <a href="https://github.com/lowkey878/lowkey-relay" target="_blank" class="text-[11px] bg-yellow-50 dark:bg-yellow-900/30 text-yellow-600 dark:text-yellow-400 px-2 py-2 rounded border border-yellow-200 dark:border-yellow-800 hover:bg-yellow-100 dark:hover:bg-yellow-900/50 transition font-black shadow-sm text-center whitespace-nowrap">ساخت پـروکـسـی شخصی</a>
                            </div>
                            <div class="relative transition-opacity duration-300 opacity-50 pointer-events-none flex-1 flex flex-col justify-start" id="user-socks5-container">
                                <input type="text" id="user-socks5-input" placeholder="socks5:// یا http:// یا (user:pass@ip:port)" dir="ltr" class="w-full px-3 py-2.5 bg-gray-50 dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-sm text-sm font-mono focus:outline-none focus:ring-2 focus:ring-yellow-500 text-gray-800 dark:text-zinc-100 transition" disabled>
                                ...
                                <div class="mt-3 p-2 border-2 border-dashed border-red-400 dark:border-red-500/70 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-300 rounded-sm text-[11px] font-bold leading-relaxed text-center w-full">
                                        پـروکـسـی‌های عمومی ناپایدارند. برای کیفیت بالاتر از <span class="text-amber-600 dark:text-amber-400 font-black">«مخزن پـروکـسـی»</span> یا از دکمه <span class="text-yellow-600 dark:text-yellow-400 font-black">«ساخت پـروکـسـی شخصی»</span> استفاده کنید.
                                </div>
                        ...
                        <div id="user-cf-proxy-section" class="transition-opacity duration-300 pt-2 border-t-2 border-gray-300 dark:border-amoled-border mt-auto">
                            ...
                            <div class="mb-2">
                                <input type="text" id="user-location-search" oninput="filterUserLocations()" placeholder="جستجوی شهر، کشور یا IATA" class="w-full px-3 py-2 bg-gray-50 dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-sm shadow-md text-sm focus:outline-none focus:ring-2 focus:ring-yellow-500 text-gray-700 dark:text-zinc-200 transition">
                            </div>
                            <div class="relative">
                                <select id="user-location-select" class="w-full pl-8 pr-3 py-2.5 bg-gray-50 dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-sm shadow-md text-sm focus:outline-none focus:ring-2 focus:ring-yellow-500 text-gray-700 dark:text-zinc-200 cursor-pointer appearance-none">
                ...
                <div class="pt-4 flex gap-3 mt-4 border-t border-gray-200 dark:border-amoled-border">
                    ...
                    <button type="submit" id="submit-btn" class="flex-1 py-3 bg-yellow-600 hover:bg-yellow-700 text-white dark:bg-yellow-600 dark:hover:bg-yellow-700 dark:text-white font-bold rounded-sm text-sm transition duration-200 shadow-md hover:shadow-sm">ایجاد کاربر</button>
                </div>
            </form>
        </div>
    </div>
<div id="ip-selector-modal" class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    <div class="w-full max-w-sm bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border border-t-2 border-t-yellow-500 rounded-sm shadow-sm overflow-hidden transition-all transform duration-300 opacity-0 scale-95 ease-out">
        ...
        <div class="p-6 space-y-4">
            ...
            <div id="ip-selection-form" class="space-y-4">
                <div>
                    ...
                    <select id="ip-operator-select" class="w-full px-3 py-2.5 bg-gray-50 dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-sm text-xs focus:outline-none focus:ring-2 focus:ring-yellow-500 text-gray-700 dark:text-zinc-300 cursor-pointer">
                ...
                <div>
                    ...
                    <input type="number" id="ip-count-input" min="1" value="20" dir="ltr" class="w-full px-3 py-2.5 bg-gray-50 dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-sm focus:outline-none focus:ring-2 focus:ring-yellow-500 text-xs font-mono text-center">
                </div>
                <div class="flex flex-col gap-2 border-t border-gray-100 dark:border-zinc-800/60 pt-3 mt-2">
                    ...
                    <div id="auto-rotate-ip-inputs-container" class="hidden transition-all duration-300 pt-1">
                        ...
                        <input type="number" id="input-auto-rotate-ip-time" min="1" placeholder="توصیه شده 5" onblur="if(this.value === '' || parseInt(this.value) < 1) this.value = '5';" class="w-full px-3 py-2.5 bg-gray-50 dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-sm focus:outline-none focus:ring-2 focus:ring-yellow-500 text-xs font-mono text-center" dir="ltr">
                    </div>
                </div>
            </div>
            <div class="pt-4 flex gap-3">
                ...
                <button type="button" onclick="applySelectedIps()" class="flex-1 py-2 bg-yellow-600 hover:bg-yellow-700 text-white dark:bg-yellow-600 dark:hover:bg-yellow-700 dark:text-white font-medium rounded-sm text-xs transition">دریافت</button>
            </div>
        </div>
    </div>
</div>
<div id="proxy-selector-modal" class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border border-t-2 border-t-yellow-500 rounded-sm shadow-sm overflow-hidden transition-all transform duration-300 opacity-0 scale-95 ease-out">
        <div class="px-6 py-4 border-b border-gray-150 dark:border-amoled-border flex justify-between items-center bg-gray-50 dark:bg-zinc-900/50">
        ...
        <div class="p-5 space-y-4">
            ...
            <div class="p-4 bg-gray-50 dark:bg-zinc-900/40 border border-gray-200 dark:border-amoled-border rounded-sm">
                ...
                <div id="proxy-loading-state" class="text-center text-[11px] text-yellow-500 font-bold hidden my-3 whitespace-pre-line leading-relaxed">
                    در حال اسکن...
                </div>
                <div id="proxy-selection-form" class="flex flex-col gap-2">
                    <select id="proxy-country-select" class="w-full px-3 py-2 bg-white dark:bg-amoled-input border border-gray-300 dark:border-zinc-700 rounded-sm text-xs focus:outline-none focus:ring-2 focus:ring-yellow-500 text-gray-700 dark:text-zinc-300 cursor-pointer">
                        <option value="">در حال آماده‌سازی...</option>
                    </select>
                    <button type="button" onclick="fetchAndLoadProxy()" id="proxy-fetch-btn" class="w-full py-2.5 bg-yellow-600 hover:bg-yellow-700 text-white dark:bg-yellow-600 dark:hover:bg-yellow-700 dark:text-white font-bold rounded-sm text-xs transition shadow-sm disabled:opacity-50 disabled:cursor-not-allowed" disabled>
...
<div id="donate-modal" class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    <div class="w-full max-w-sm bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border border-t-2 border-t-yellow-500 rounded-sm shadow-sm overflow-hidden transition-all transform duration-300 opacity-0 scale-95 ease-out" id="donate-modal-card">
        <div class="px-6 py-4 border-b border-gray-150 dark:border-amoled-border flex justify-between items-center bg-gray-50 dark:bg-zinc-900/50">
        ...
        <div class="p-6 space-y-4">
            <p class="text-[11px] text-gray-600 dark:text-zinc-400 leading-relaxed font-medium">
                اگر سرور دارید میتونید با دکمه <span class="text-yellow-600 dark:text-yellow-400 font-black">«ساخت پـروکـسـی شخصی»</span> یک پـروکـسـی بسازید و اهدا کنید به پروژه
            </p>
            <div>
    ...
    <div id="settings-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
        <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border border-t-2 border-t-yellow-500 rounded-sm shadow-sm overflow-hidden transition-all transform duration-300 opacity-0 scale-95 ease-out flex flex-col max-h-[90vh]">
            <div class="px-6 py-4 border-b border-gray-150 dark:border-amoled-border flex justify-between items-center bg-gray-50 dark:bg-zinc-900/50">
            ...
            <div class="p-6 space-y-4 overflow-y-auto flex-1 overscroll-contain">
                <div class="pt-2">
                    ...
                    <div class="relative">
                        <select id="refresh-rate-select" onchange="changeRefreshRate(this.value)" class="w-full pl-8 pr-3 py-2.5 bg-white dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-sm text-sm focus:outline-none focus:ring-2 focus:ring-yellow-500 text-gray-700 dark:text-zinc-200 cursor-pointer appearance-none">
                ...
                <div class="pt-4 border-t-2 border-gray-300 dark:border-zinc-700">
                    ...
                    <div class="space-y-3">
                        <div>
                            ...
                            <input type="password" id="change-pwd-current" class="w-full px-3 py-2 bg-white dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-sm focus:outline-none focus:ring-2 focus:ring-yellow-500 text-xs font-mono text-center">
                        </div>
                        <div>
                            <label class="block text-[11px] text-gray-500 dark:text-gray-400 font-medium mb-1">رمز عبور جدید</label>
                            <input type="password" id="change-pwd-new" class="w-full px-3 py-2 bg-white dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-sm focus:outline-none focus:ring-2 focus:ring-yellow-500 text-xs font-mono text-center">
                        </div>
                        <button type="button" onclick="changeAdminPassword()" id="change-pwd-btn" class="w-full py-2 bg-yellow-600 hover:bg-yellow-700 text-white dark:bg-yellow-600 dark:hover:bg-yellow-700 dark:text-white font-semibold rounded-sm text-xs transition-all shadow-sm">تغییر رمز عبور</button>
                    </div>
                </div>
                <div class="pt-4 border-t-2 border-gray-300 dark:border-zinc-700">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <button type="button" onclick="triggerImportBackup()" class="py-2.5 bg-transparent border-2 border-yellow-500 text-yellow-600 hover:bg-yellow-50 dark:text-yellow-400 dark:border-yellow-500/60 dark:hover:bg-yellow-500/10 rounded-sm text-xs font-bold transition flex items-center justify-center gap-1.5 shadow-sm">
                            📥 بازیابی
                        </button>
                    </div>
                ...
                <div class="pt-4 flex gap-3">
                    ...
                    <button type="button" onclick="saveSettings()" id="save-settings-btn" class="flex-1 py-2 bg-yellow-600 hover:bg-yellow-700 text-white dark:bg-yellow-600 dark:hover:bg-yellow-700 dark:text-white font-medium rounded-sm text-sm transition">ذخیره تنظیمات</button>
                </div>
            </div>
        </div>
    </div>
<div id="qr-modal" class="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/70 opacity-0 pointer-events-none transition-opacity duration-200 ease-out">
    <div id="qr-modal-card" class="w-full max-w-sm bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border border-t-2 border-t-yellow-500 rounded-sm shadow-md p-6 transform transition-all scale-95 opacity-0 duration-200 text-center">
    ...
    <div id="bulk-actions-bar" class="fixed bottom-4 left-1/2 -translate-x-1/2 z-[40] bg-white dark:bg-zinc-900/90 border border-gray-200 dark:border-zinc-800/80 px-6 py-4 rounded-sm shadow-md flex flex-wrap items-center justify-between gap-4 w-[95%] max-w-4xl transition-all duration-300 transform translate-y-28 opacity-0 pointer-events-none backdrop-blur-md">
        <div class="flex items-center gap-2">
            <span class="w-3 h-3 bg-yellow-500 rounded-full animate-pulse shadow-sm shadow-yellow-500/50"></span>
        ...
        <div class="flex flex-wrap gap-2 justify-end">
            ...
            <button onclick="bulkReset('volume')" class="px-3 py-1.5 bg-yellow-50 dark:bg-yellow-950/20 text-yellow-600 dark:text-yellow-400 hover:bg-yellow-100 dark:hover:bg-yellow-900/30 rounded-sm text-xs font-bold transition border border-yellow-200 dark:border-yellow-900/50 flex items-center gap-1">
                📊 ریست حجم
            </button>
...
<div id="custom-confirm-modal" class="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    <div id="custom-confirm-card" class="w-full max-w-sm bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border border-t-2 border-t-yellow-500 rounded-sm shadow-md overflow-hidden p-6 text-center transform transition-all scale-95 duration-300">
        ...
        <div class="flex gap-3">
            ...
            <button id="custom-confirm-ok" class="flex-1 py-3 bg-yellow-600 hover:bg-yellow-700 text-white dark:bg-yellow-600 dark:hover:bg-yellow-700 dark:text-white font-bold rounded-sm text-sm transition duration-200 shadow-sm">تأیید</button>
        </div>
    </div>
</div>
    <script>
        ...
        function renderPortCheckboxes() {
            ...
            tlsContainer.innerHTML = tlsPorts.map(function(port) {
                ...
                return '<label class="relative cursor-pointer">' +
                    ...
                    '<div class="flex items-center justify-center gap-1 px-1.5 py-1.5 border border-gray-200 dark:border-zinc-800/80 rounded-sm text-[11px] font-semibold select-none transition-all duration-200 hover:bg-gray-50 dark:hover:bg-zinc-800/40 text-gray-700 dark:text-zinc-300 peer-checked:bg-yellow-50 dark:peer-checked:bg-yellow-950/25 peer-checked:border-yellow-500 dark:peer-checked:border-yellow-500/70 peer-checked:text-yellow-600 dark:peer-checked:text-yellow-400 shadow-sm">' +
                        '<span>' + port + '</span>' +
                        '<svg class="w-3 h-3 hidden peer-checked:block text-yellow-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"></path></svg>' +
                    '</div>' +
                '</label>';
            }).join('');
                ...
                tbody.innerHTML = users.map(user => {
					...
					if (user.limit_req) {
					    ...
					    reqHtml = '<div class="flex flex-col gap-1.5 w-full min-w-[65px] max-w-[90px] mx-auto select-none">' +
					        ...
					        '</div>' +
					        '<div class="w-full h-1.5 bg-gray-200 dark:bg-zinc-700 rounded-full overflow-hidden">' +
					            '<div class="w-full h-full bg-yellow-500 rounded-full transition-all duration-500"></div>' +
					        '</div>' +
					    '</div>';
					}
					let volumeHtml = '';
                    ...
                    return '<tr class="hover:bg-gray-50 dark:hover:bg-zinc-900/40 border-b border-gray-100 dark:border-zinc-800 last:border-0">' +
                            '<td class="p-1 border-r border-gray-100 dark:border-zinc-800 text-center select-none">' +
                                '<input type="checkbox" name="select-user" value="' + encodeURIComponent(user.username) + '" onchange="onUserSelectChange(this)" ' + isChecked + ' class="w-4 h-4 rounded-sm border-2 border-gray-300 dark:border-zinc-700 text-yellow-600 bg-white dark:bg-zinc-800 checked:bg-yellow-600 checked:border-yellow-600 focus:ring-yellow-500/50 focus:ring-offset-0 transition-all duration-200 cursor-pointer hover:scale-105 active:scale-95">' +
                            '</td>' +
                            ...
                            '<td class="p-1.5 border-r border-gray-100 dark:border-zinc-800 text-center">' +
                                '<div class="grid grid-cols-2 gap-1 w-max mx-auto">' +
                                    '<button onclick="copyConfig(\\'' + encodeURIComponent(user.username) + '\\')" title="کپی کـانفـیگ" class="p-1 flex items-center justify-center bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 hover:bg-yellow-50 dark:hover:bg-yellow-900/30 text-yellow-600 dark:text-yellow-400 rounded transition shadow-sm"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg></button>' +
                            ...
                            '<td class="p-1 border-r border-gray-100 dark:border-zinc-800">' +
							    '<div class="flex flex-col gap-0.5 w-[90px] mx-auto">' +
							        '<button onclick="copySubLink(\\'' + encodeURIComponent(user.username) + '\\')" class="w-full flex items-center justify-center gap-1 px-1 py-0.5 bg-yellow-50 dark:bg-yellow-900/30 text-yellow-600 dark:text-yellow-400 hover:bg-yellow-100 dark:hover:bg-yellow-900/50 rounded text-[9px] font-bold transition border border-yellow-200 dark:border-yellow-800">' +
							...
							'<td class="p-1 border-r border-gray-100 dark:border-zinc-800 text-xs">' + 
							    '<div class="grid grid-flow-col grid-rows-3 gap-1 w-max mx-auto">' +
							        String(user.port || "").split(",").map(function(p) {
							            ...
							            var colorClass = isTls ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400' : 
...
function setModalState(modalId, show) {
		...
		function getvIeesLink(username) {
            ...
            const links = [];
		const dynPath = encodeURIComponent("/stream/lowkey878/" + (user.uuid ? user.uuid.split("-")[0] : "default"));
		const m2 = decodeURIComponent('%E2%9C%A8%20%40lowkey878%20%E2%9C%A8');
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>پنل لوکی - وضعیت اشتراک</title>
    ${COMMON_HEAD}
    <style>
        body { font-family: 'Vazirmatn', sans-serif; }
        .glass {
        ...
        .u-ring {
            ...
            box-shadow: 0 0 0 4px rgba(245,194,66,0.08), 0 0 30px rgba(245,194,66,0.12);
        }
        .u-ring::after {
            content: attr(data-label);
            position: absolute; inset: 8px; border-radius: 50%;
            background: #0b0b0f; display: flex; align-items: center; justify-content: center;
            font-size: 0.8rem; font-weight: 800; color: var(--ring-color);
            border: 1px solid rgba(255,255,255,0.06);
        }
        .tab-btn.active {
            background: linear-gradient(135deg, rgba(245,194,66,0.18), rgba(6,182,212,0.12));
        ...
        .os-body { display: none; padding: 0.85rem; }
        .os-card.open .os-body { display: block; }
        .os-card.open .chev { transform: rotate(180deg); }
        .chev { transition: transform 0.2s; }
        .cfg-card { transition: border-color .15s ease; }
        .cfg-card:hover { border-color: #f5c242; }
    </style>
</head>
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen py-6 px-3 overflow-x-hidden">
<div class="w-full max-w-xl mx-auto">

    <div class="flex items-center justify-between gap-3 px-4 py-3 mb-3 bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-sm shadow-sm">
        <div class="flex items-center gap-2.5 min-w-0">
            <div class="p-2 bg-yellow-950/60 border border-yellow-500 text-yellow-400 rounded-sm shadow-[0_0_15px_rgba(245,194,66,0.35)] shrink-0">
            ...
            <div class="min-w-0">
                <p class="text-sm font-bold text-gray-900 dark:text-white leading-tight truncate">پـنـل لوکی</p>
                <p class="text-[10px] text-gray-500 dark:text-zinc-400">وضعیت اشتراک</p>
            </div>
        </div>
        <a href="https://t.me/lowkey878" target="_blank" class="flex items-center gap-1.5 px-3 py-1.5 bg-cyan-950/20 dark:bg-cyan-950/30 border border-cyan-800/40 text-cyan-500 dark:text-cyan-400 rounded-full text-[11px] font-bold hover:bg-cyan-900/30 transition shrink-0">
    ...
    <div class="glass rounded-sm shadow-md border-t-2 border-t-yellow-500 p-5 relative overflow-hidden mb-3">
        <div class="absolute -left-12 -top-12 w-40 h-40 bg-yellow-500/10 rounded-full blur-3xl pointer-events-none"></div>
        ...
        <div class="relative z-10 flex items-center gap-4 mb-4">
            <div class="u-ring" id="usage-ring" data-label="۰٪"></div>
            <div class="min-w-0 flex-1">
                <p id="display-username" class="text-lg font-bold text-gray-900 dark:text-white truncate font-mono"></p>
                <p id="display-flag" class="text-xs text-gray-500 dark:text-zinc-400 mb-1.5" style="display:none;"></p>
                <div id="status-chip" class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold border">
                    <span id="status-dot" class="w-1.5 h-1.5 rounded-full bg-current"></span>
                    <span id="status-text">در حال بارگذاری...</span>
                </div>
            </div>
        </div>

        <div class="grid grid-cols-2 gap-2.5 relative z-10 mb-4">
            <div class="bg-white/40 dark:bg-zinc-900/30 border border-gray-200 dark:border-amoled-border rounded-sm p-3 shadow-sm">
        ...
        <div class="rounded-sm p-3 bg-gradient-to-br from-yellow-500/10 to-cyan-500/10 border border-yellow-500/20 relative z-10">
            <label class="text-[10px] font-bold text-yellow-600 dark:text-yellow-500 uppercase tracking-wide flex items-center gap-1">⛓️ لینک اشتراک (خودکار)</label>
            <input id="sub-url-input" readonly dir="ltr" onclick="this.select()" class="w-full mt-2 px-2.5 py-2 bg-black/5 dark:bg-black/40 border border-gray-300 dark:border-zinc-700 rounded-sm text-[11px] font-mono text-gray-700 dark:text-cyan-300 truncate" />
            <div class="flex gap-2 mt-2">
                <button onclick="copyTextSub()" class="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 bg-yellow-500 hover:bg-yellow-400 text-black rounded-sm text-xs font-bold shadow-sm transition">کپی لینک</button>
                <button onclick="showSubQr()" class="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 bg-cyan-600 hover:bg-cyan-500 text-white rounded-sm text-xs font-bold shadow-sm transition">📱 QR کد</button>
            </div>
        </div>
    </div>

    <div class="flex gap-1 p-1 mb-3 bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-sm shadow-sm">
        <button class="tab-btn active flex-1 py-2 rounded-sm text-xs font-bold text-gray-600 dark:text-zinc-400 transition" data-tab="configs" onclick="switchTab('configs')">🚀 کـانفـیگ‌ها</button>
        <button class="tab-btn flex-1 py-2 rounded-sm text-xs font-bold text-gray-600 dark:text-zinc-400 transition" data-tab="guide" onclick="switchTab('guide')">📲 راهنمای اتصال</button>
    </div>

    <div id="tab-configs" class="tab-panel active">
        <div class="flex justify-between items-center mb-2">
            <span class="text-xs text-gray-500 dark:text-zinc-400">هر کانفیگ مربوط به یک لوکیشن/پورت است</span>
            <button onclick="copyAllConfigs()" class="text-[11px] font-bold text-yellow-600 dark:text-yellow-500 hover:underline">کپی همه</button>
        </div>
        <div id="configs-list" class="space-y-2"></div>
    </div>

    <div id="tab-guide" class="tab-panel">
        <div id="os-root" class="space-y-2"></div>
    </div>
...
${COMMON_TOAST_HTML}
    <script>
        ...
        function getSubUrl() {
            return window.location.protocol + '//' + getHost() + '/sub/' + encodeURIComponent(window.statusUser.username);
        }
        function getFlagEmoji(countryCode) {
            if (!countryCode) return '🌐';
            const codePoints = countryCode.toUpperCase().split('').map(function(char){ return 127397 + char.charCodeAt(0); });
            try {
        ...
        function getResolvedFlag() {
            ...
            if (u.user_proxy_iata) {
                try {
                    ...
                    if (cachedLocations) {
                        const parsedLocs = JSON.parse(cachedLocations);
                        const loc = parsedLocs.find(function(l){ return l.iata && l.iata.toUpperCase() === u.user_proxy_iata.toUpperCase(); });
                        if (loc && loc.cca2) flagEmoji = getFlagEmoji(loc.cca2);
                    }
                } catch(e) {}
        ...
        function getvIeesLinks() {
            ...
            ips.forEach(function(ip, ipIndex) {
                ports.forEach(function(portStr) {
                    ...
                    var link = 'vle' + 'ss://' + (u.uuid || '') + '@' + ip + ':' + portStr + '?path=' + dynPath + '&security=' + tlsVal + '&encryption=none&insecure=0&host=' + host + '&fp=' + fp + '&type=ws&allowInsecure=0&sni=' + host + userFrag + '#' + encodeURIComponent(remark);
                    items.push({ ip: ip, port: portStr, tls: isTlsPort, link: link, flag: flagEmoji });
                });
            });
            return items;
        }
        function copyAllConfigs() {
            const items = getvIeesLinks();
            const text = items.map(function(i){ return i.link; }).join('\\n');
            navigator.clipboard.writeText(text).then(function(){ showToast('✅ همه کانفیگ‌ها کپی شد!'); });
        }
        function copyOneConfig(idx) {
            const items = getvIeesLinks();
            const item = items[idx];
            if (!item) return;
            navigator.clipboard.writeText(item.link).then(function(){ showToast('✅ کانفیگ کپی شد!'); });
        }
        function showConfigQr(idx) {
            const items = getvIeesLinks();
            const item = items[idx];
            if (!item) return;
            toggleQrModal(true, item.link);
        }
        function renderConfigsList() {
            const items = getvIeesLinks();
            const list = document.getElementById('configs-list');
            list.innerHTML = items.map(function(item, idx){
                return '<div class="cfg-card flex items-center justify-between gap-2 px-3 py-2.5 bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-sm shadow-sm">' +
                    '<div class="flex items-center gap-2 min-w-0">' +
                        '<span class="text-lg shrink-0">' + item.flag + '</span>' +
                        '<div class="min-w-0">' +
                            '<p class="text-xs font-bold text-gray-800 dark:text-zinc-200 truncate" dir="ltr">' + item.ip + ':' + item.port + '</p>' +
                            '<p class="text-[10px] text-gray-500 dark:text-zinc-400">vless · ' + (item.tls ? 'tls' : 'none') + '</p>' +
                        '</div>' +
                    '</div>' +
                    '<div class="flex gap-1.5 shrink-0">' +
                        '<button onclick="copyOneConfig(' + idx + ')" class="px-2.5 py-1.5 bg-yellow-500 hover:bg-yellow-400 text-black rounded-sm text-[11px] font-bold">کپی</button>' +
                        '<button onclick="showConfigQr(' + idx + ')" class="px-2.5 py-1.5 bg-cyan-600 hover:bg-cyan-500 text-white rounded-sm text-[11px] font-bold">QR</button>' +
                    '</div>' +
                '</div>';
            }).join('');
        }
        function copyTextSub() {
            const link = getSubUrl();
            navigator.clipboard.writeText(link).then(function(){ showToast('✅ لینک ساب متنی کپی شد!'); });
        }
        function showSubQr() {
            toggleQrModal(true, getSubUrl());
        }
        function toggleQrModal(show, text) {
        ...
        function osCard(id, headGradient, title, recommended, bodyHtml, openDefault) {
            return '<div class="os-card rounded-sm border border-gray-200 dark:border-amoled-border overflow-hidden shadow-sm' + (openDefault ? ' open' : '') + '" id="os-' + id + '">' +
                '<button type="button" onclick="document.getElementById(\\'os-' + id + '\\').classList.toggle(\\'open\\')" class="w-full flex items-center justify-between px-4 py-3 text-white font-bold text-sm" style="background:' + headGradient + '">' +
                    '<span class="flex items-center gap-2">' + title + (recommended ? '<span class="text-[10px] font-bold px-1.5 py-0.5 bg-white/25 rounded-sm">پیشنهادی</span>' : '') + '</span>' +
                    '<span class="chev">▼</span>' +
                '</button>' +
                '<div class="os-body bg-gray-50 dark:bg-amoled-card">' + bodyHtml + '</div>' +
            '</div>';
        }
        function stepBlock(num, title, body) {
            return '<div class="mb-3 pr-3 border-r-2 border-yellow-500">' +
                '<p class="text-[10px] font-bold text-yellow-600 dark:text-yellow-500 mb-0.5">مرحله ' + num + '</p>' +
                '<h4 class="text-xs font-bold text-gray-800 dark:text-zinc-200 mb-1">' + title + '</h4>' +
                body +
            '</div>';
        }
        function actionLink(href, label) {
            return '<a href="' + href + '" class="block w-full text-center px-3 py-2 mb-1.5 bg-white dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-sm text-[11px] font-bold text-gray-700 dark:text-zinc-300 hover:border-yellow-500 transition">' + label + '</a>';
        }
        function storeLink(href, label) {
            return '<a href="' + href + '" target="_blank" rel="noopener" class="block w-full text-center px-3 py-2 mb-1.5 bg-cyan-950/10 dark:bg-cyan-950/30 border border-cyan-700/30 rounded-sm text-[11px] font-bold text-cyan-700 dark:text-cyan-400 hover:border-cyan-500 transition">' + label + ' ↓</a>';
        }
        function renderGuide() {
            const root = document.getElementById('os-root');
            if (!root) return;
            const fullUrl = getSubUrl();
            ...
            const iosBody =
                stepBlock('۱', 'انتخاب اپلیکیشن', '<p class="text-[11px] text-gray-500 dark:text-zinc-400 mb-2">Streisand، V2Box یا Shadowrocket</p>') +
                stepBlock('۲', 'دانلود اپلیکیشن', storeLink('https://apps.apple.com/us/app/streisand/id6450534064', 'دانلود Streisand') + storeLink('https://apps.apple.com/us/app/v2box-v2ray-client/id6446814690', 'دانلود V2Box')) +
                stepBlock('۳', 'افزودن اشتراک', actionLink('streisand://import/' + fullUrl, 'افزودن در Streisand ←') + actionLink('v2box://install-sub?url=' + enc + '&name=lowkey878', 'افزودن در V2Box ←') + actionLink('sub://' + b64, 'افزودن در Shadowrocket ←'));

            const androidBody =
                stepBlock('۱', 'انتخاب اپلیکیشن', '<p class="text-[11px] text-gray-500 dark:text-zinc-400 mb-2">v2rayNG، NekoBox یا sing-box</p>') +
                stepBlock('۲', 'دانلود اپلیکیشن', storeLink('https://play.google.com/store/apps/details?id=com.v2ray.ang', 'v2rayNG (Google Play)') + storeLink('https://github.com/2dust/v2rayNG/releases/latest', 'v2rayNG (APK مستقیم)')) +
                stepBlock('۳', 'افزودن اشتراک', actionLink('v2rayng://install-config?url=' + enc, 'افزودن در v2rayNG ←') + actionLink('sn://subscription?url=' + enc + '&name=lowkey878', 'افزودن در NekoBox ←') + actionLink('sing-box://import-remote-profile?url=' + enc + '#lowkey878', 'افزودن در sing-box ←'));

            const winBody =
                stepBlock('۱', 'انتخاب اپلیکیشن', '<p class="text-[11px] text-gray-500 dark:text-zinc-400 mb-2">Clash Verge یا sing-box</p>') +
                stepBlock('۲', 'دانلود اپلیکیشن', storeLink('https://github.com/clash-verge-rev/clash-verge-rev/releases/latest', 'دانلود Clash Verge')) +
                stepBlock('۳', 'افزودن اشتراک', actionLink('clash://install-config?url=' + enc, 'افزودن در Clash Verge ←'));

            const cards = [
                { id: 'ios', html: osCard('ios', 'linear-gradient(135deg,#06b6d4,#0284c7)', '📱 iOS / macOS', true, iosBody, isIOS) },
                { id: 'android', html: osCard('android', 'linear-gradient(135deg,#22c55e,#15803d)', '🤖 اندروید', false, androidBody, isAndroid) },
                { id: 'win', html: osCard('win', 'linear-gradient(135deg,#f5c242,#d97706)', '🖥️ ویندوز', false, winBody, !isIOS && !isAndroid) }
            ];
            var order = isAndroid ? ['android','ios','win'] : ['ios','android','win'];
            root.innerHTML = order.map(function(id){ return cards.find(function(c){ return c.id === id; }).html; }).join('');
        }
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('sub-url-input').value = getSubUrl();
            ...
            if (u.user_proxy_iata) {
                ...
                .then(function(data){
                    flagContainer.innerText = (data.success && data.country) ? getFlagEmoji(data.country) : "🌐";
                })
                .catch(function(){ flagContainer.innerText = "🌐"; });
            }
            ...
            if (limitGb) {
                ...
                volumePct = Math.min((usedGb / limitGb) * 100, 100);
                document.getElementById('volume-pct').innerText = volumePct.toFixed(0) + '٪';
                document.getElementById('volume-progress').style.width = volumePct + '%';
                const hue = 120 - (volumePct * 1.2);
                ...
                document.getElementById('volume-progress').style.backgroundColor = '#f5c242';
            }
            let daysRemaining = 'نامحدود';
            ...
            if (u.expiry_days) {
                ...
                if (u.created_at) {
                    ...
                    const diffDays = Math.ceil((expiryDate - new Date()) / (1000 * 60 * 60 * 24));
                    daysRemaining = diffDays > 0 ? diffDays : 0;
                    expiryPct = Math.max(0, Math.min(100, (daysRemaining / u.expiry_days) * 100));
                    document.getElementById('expiry-pct').innerText = expiryPct.toFixed(0) + '٪';
                    document.getElementById('expiry-progress').style.width = expiryPct + '%';
                    const hue = expiryPct * 1.2;
                ...
                document.getElementById('expiry-progress').style.backgroundColor = '#f5c242';
            }
            ...
            if (limitReq) {
                ...
                document.getElementById('req-progress').style.backgroundColor = '#f5c242';
            }
            ...
            if (u.is_active === 0) {
                statusChip.className = 'inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold border bg-red-500/10 border-red-500/30 text-red-500';
                statusText.innerText = 'غیرفعال / مسدود دستی';
                chipColor = '#ef4444';
            } else if (isVolumeExpired || isReqExpired || isTimeExpired) {
                statusChip.className = 'inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold border bg-yellow-500/10 border-yellow-500/30 text-yellow-600 dark:text-yellow-500';
                if (isVolumeExpired) statusText.innerText = 'تمام شدن حجم مجاز';
                else if (isReqExpired) statusText.innerText = 'تمام شدن ریکوئست مجاز';
                else if (isTimeExpired) statusText.innerText = 'منقضی شده';
                chipColor = '#f5c242';
            } else {
                statusChip.className = 'inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold border bg-green-600/10 border-green-600/30 text-green-600';
                statusText.innerText = 'فعال و متصل';
                chipColor = '#16a34a';
            }
            const ring = document.getElementById('usage-ring');
            const ringPct = limitGb ? volumePct : (100 - expiryPct);
            ring.style.setProperty('--pct', ringPct);
            ring.setAttribute('data-label', ringPct.toFixed(0) + '٪');
            ring.style.setProperty('--ring-color', u.is_active === 0 ? '#ef4444' : (ringPct >= 80 ? '#ef4444' : (ringPct >= 45 ? '#f5c242' : '#22d3ee')));
            renderConfigsList();
            renderGuide();
        });
